...
- [ ] **G10.1** `DetailLayout` + `useAiJob` + `AIResultPanel`; детальная страница кандидата (`/candidates/:id`) — Сводка/Резюме/Отклики; реестр → navigate.
- [ ] **G10.2** M1-панель (критерии + скрининг run+poll + таблица + Подтвердить/Оспорить) — кандидат (Оценка) и вакансия (Критерии).
- [ ] **G10.3** M2-панель (риски run+poll + signals + бейдж) — кандидат (Риски).
- [ ] **G10.4** M3-панель (вопросы run+poll + карточки + утверждение) — кандидат (Вопросы).
- [ ] **G10.5** Детальная страница вакансии (Описание/Критерии/Воронка-канбан DnD/Карта поиска M4/Аналитика/Планировщик M6/Настройки).
- [ ] **G10.6** Детальная страница отклика (таймлайн + быстрые действия + вкладки M1/M2 + M6).
- [ ] **G10.7** Аналитика: воронка-санкей, портфель вакансий, ИИ-операции; полировка.
