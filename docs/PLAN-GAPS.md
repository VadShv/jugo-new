...
- [ ] `AnalyticsPage`: `GET /api/v1/analytics/funnel/{vacancy_id}` (выбор вакансии), `/analytics/sources`, `/analytics/ai` → ECharts с реальными данными.
- [ ] Селектор вакансии для воронки.
- **Приёмка**: графики строятся из реальных данных (совпадает с E2E: 1 отклик, in_progress).
- **Зависимости**: G2, G4.

**Гейт P0**: фронтенд полностью работает против бэкенда (логин → реестры → поиск → создание → переход → аналитика). Деплой обновлён.
